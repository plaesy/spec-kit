...
>
> **At scale** (roughly 20+ functional requirements): don't let this section grow
> unbounded in one file. Group requirements by module/subsystem, keep this
> section as a summary index (ID + title + one-line description + link), and
> move full per-requirement detail to `srs/FR-<module>.md` files — the same
> index-links-to-topic-files pattern used by `.plaesy/memory.md` and, past 15
> components, `docs/components.md` (see `/doc`).
