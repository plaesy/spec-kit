...
**See also**: `assess-financial.instructions.md` (Financial dimension), `assess-marketing.instructions.md` (Marketing dimension) — split out as dedicated files; this file retains its own Marketing/Financial component-scoring tables below for backward reference only.
...
1. **Confidence Levels**:
   - HIGH: Based on actual market data + customer feedback + validated metrics
   - MEDIUM: Based on pattern detection + competitive analysis + partial data
   - LOW: Based on incomplete data or specification gaps
...
4. **Speculative vs Confirmed**:
   - CONFIRMED: [Based on specific market data or customer evidence]
   - LIKELY: [Pattern matches, but unconfirmed by primary research]
   - POSSIBLE: [Could happen IF [condition]]
