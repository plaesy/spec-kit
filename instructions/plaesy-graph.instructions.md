...
- Before large refactors of `.plaesy/memory/`, `.plaesy/instructions/`, `.plaesy/roles/`, or `.plaesy/scripts/`, to check what would be affected.
...
4. The two scripts' outputs are independent (each rescans and overwrites `.plaesy/analysis/`) — don't mix
...
1. `-SemanticQueue` dumps every INFERRED edge to `.plaesy/analysis/semantic-queue.json`.
