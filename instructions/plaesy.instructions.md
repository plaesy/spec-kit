...
### Global Routing Logic (Applied Everywhere)

```
ANY WORKFLOW OUTPUT 
  ↓
CATEGORIZE BY DIMENSION:
├─ Technical (code, tests, security, performance, infra)
├─ Design (UI/UX, accessibility, systems)
├─ Business (market fit, viability, model)
├─ Product (features, roadmap, competitive)
├─ Marketing (positioning, messaging, GTM)
├─ Operations (team, processes, capacity)
├─ Legal (compliance, privacy, risk)
└─ Financial (pricing, cost, profitability)
  ↓
ANALYZE FINDINGS:
├─ Severity: CRITICAL, HIGH, MEDIUM, LOW
├─ Impact: Business, Technical, User-facing, Internal
├─ Type: Issue, Risk, Opportunity, Gap
└─ Fixability: Auto-fixable, Needs review, Blocked
  ↓
AUTONOMOUS ROUTING RULES:
IF critical security/legal → BLOCK & route to /fix (immediate)
ELSE IF technical gap → route to /implement (TDD)
ELSE IF design issue → route to /optimize (redesign/refactor per /assess Design Spine)
ELSE IF business issue → route to /assess:business (research)
ELSE IF performance issue → route to /optimize (improve)
ELSE IF low priority → route to /loop (batch fix)
ELSE IF missing docs → route to /doc (generate)
  ↓
EXECUTE ROUTED PHASE
  ↓
RE-ASSESS AFTER FIX
  ↓
...
**Key Principle**: Each output automatically knows its next home. Users never ask "what now?"

### Dimension-Based Autonomous Decisions

**Technical Issues**:
- Test gap (coverage <90%) → `/implement --tdd`
- Performance (>200ms) → `/optimize --performance`
- Security vulnerability → `/fix --security`
- Code quality → `/loop --quality`
...
- Revenue model unclear → `/assess:business --validate`
- Competitive threat → `/assess:product --competitive`

**Operations Issues**:
- Team capacity → `/assess:management`
- Process inefficiency → `/assess:management --process`
- Scalability concern → `/optimize --scalability`

**Legal/Compliance Issues**:
- Regulatory gap → `/fix --compliance`
- Privacy issue → `/fix --privacy`
- Risk identified → `/assess:legal --risk`

**Financial Issues**:
- Cost overrun → `/assess:financial`
- Pricing unclear → `/assess:business --financial`

---

## Workflow Phases

Executes work through 9 sequential phases across **ALL scopes** (technical, design, business, product, marketing, operations, legal, financial). Each phase has a dedicated command (`/start` orchestrates all; individual commands available for resume/override).
