...
update_agent_file() { local target_file="$1" agent_name="$2"; echo "Updating $agent_name context file: $target_file"; local temp_file=$(mktemp); if [ ! -f "$target_file" ]; then
  echo "Creating new $agent_name context file..."; if [ -f "$REPO_ROOT/.plaesy/templates/agent-file-template.md" ]; then cp "$REPO_ROOT/.plaesy/templates/agent-file-template.md" "$temp_file"; else echo "ERROR: Template not found"; return 1; fi;
