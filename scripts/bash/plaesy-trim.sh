...
#   plaesy-trim.sh compress --path .plaesy/instructions/plaesy.md
#   plaesy-trim.sh compress --path .plaesy/memory --recurse --dry-run
#   plaesy-trim.sh compress --path README.md --level ultra
#   plaesy-trim.sh llm-queue --path .plaesy/instructions/plaesy.md
#   plaesy-trim.sh apply-llm --path .plaesy/instructions/plaesy.md --annotations annotations.json
...
GRAPH_PATH="$REPO_ROOT/.plaesy/analysis/project.graph.json"
...
apply_level() {
    ...
    if [[ "$level" == "full" || "$level" == "ultra" ]]; then
        ...
        # Match plaesy-trim.ps1's Compress-Prose: strip leading whitespace from
        # every line (e.g. indented list-continuation lines) at full/ultra.
        piece="$(printf '%s' "$piece" | sed -E 's/^[[:space:]]+//')"
    fi
    if [[ "$level" == "ultra" ]]; then
...
cmd_llm_queue() {
    ...
    local out_dir="$REPO_ROOT/.plaesy/analysis"
    mkdir -p "$out_dir"
