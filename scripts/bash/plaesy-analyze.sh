...
detect_all_frameworks() {
    ...
    # C#/.NET frameworks
    if compgen -G "$PROJECT_PATH/*.csproj" > /dev/null; then
...
detect_development_tools() {
    ...
    if [[ -f "$PROJECT_PATH/kubernetes" ]] || [[ -d "$PROJECT_PATH/kubernetes" ]] || { compgen -G "$PROJECT_PATH/*.yaml" > /dev/null && grep -q "apiVersion:" "$PROJECT_PATH"/*.yaml 2>/dev/null; }; then
        tools+=("Kubernetes")
    fi
...
generate_project_json() {
    ...
    # Use cached file type counts (calculated once in main())
    local sc_count="$FILE_COUNTS_SC"
    local doc_count="$FILE_COUNTS_DOC"
    local cfg_count="$FILE_COUNTS_CFG"
...
generate_project_structure_json() {
    ...
    # Uses -print (portable to macOS) instead of GNU -printf
    local -A file_counts_map
    while IFS=$'\t' read -r count_dir count; do
        [[ -n "$count_dir" ]] && file_counts_map["$count_dir"]=$count
    done < <(find "$PROJECT_PATH" -type f ! -path "*/.*/*" ! -path "*/node_modules/*" -print 2>/dev/null | awk -F/ '{ d=$0; sub(/\/[^\/]*$/, "", d); c[d]++ } END { for (d in c) print d "\t" c[d] }')
    ...
    # Get key files: find pass that works on both Linux and macOS
    # Uses portable -print flag and stat for size (compatible with BSD stat on macOS)
    local key_files
    key_files=$(find "$PROJECT_PATH" -type f \( \
        ...
        \) ! -path "*/.*/*" ! -path "*/node_modules/*" -print 2>/dev/null | while read -r f; do
            # Try to get file size; fallback to 0 if stat unavailable (very rare)
            local fsize=0
            if [[ -f "$f" ]]; then
                if stat -f%z "$f" &>/dev/null; then
                    fsize=$(stat -f%z "$f" 2>/dev/null || echo 0)  # BSD stat (macOS)
                else
                    fsize=$(stat -c%s "$f" 2>/dev/null || echo 0)  # GNU stat (Linux)
                fi
            fi
            echo "$fsize	$f"
        done | sort -t$'\t' -k2)
...
# Function to count file types across project (single traversal)
# Outputs three counts separated by newlines: source_code, documentation, configuration
count_file_types() {
    local sc_count=0 doc_count=0 cfg_count=0
    while IFS= read -r fpath; do
        # Extract filename (basename) for case matching, compatible with macOS
        local fname="${fpath##*/}"
        case "$fname" in
            *.js|*.jsx|*.ts|*.tsx|*.py|*.go|*.java|*.dart|*.sh|*.bash|*.ps1|*.psm1|*.rb|*.php|*.rs|*.c|*.cc|*.cpp|*.h|*.hpp|*.cs|*.kt|*.kts|*.swift) sc_count=$((sc_count + 1)) ;;
            *.md|*.txt|*.rst|*.adoc) doc_count=$((doc_count + 1)) ;;
            *.json|*.yaml|*.yml|*.toml|*.ini|*.xml|*.cfg) cfg_count=$((cfg_count + 1)) ;;
        esac
    done < <(find "$PROJECT_PATH" -type f ! -path "*/.*/*" ! -path "*/node_modules/*" -print 2>/dev/null)
    echo "$sc_count"
    echo "$doc_count"
    echo "$cfg_count"
}
...
generate_analysis_overview_md() {
    local target_file="$PROJECT_PATH/.plaesy/analysis/overview.md"
    log_info "Generating analysis/overview.md (replacing previous snapshot)..."
    ...
    local recommendations=""
    [ "$doc_count" -lt 3 ] && recommendations+="- Add comprehensive documentation (only $doc_count doc file(s) found)\n"
    if ! find "$PROJECT_PATH" -type d \( -iname "test*" -o -iname "spec*" -o -iname "__tests__" \) ! -path "*/.*/*" ! -path "*/node_modules/*" 2>/dev/null | grep -q .; then
        recommendations+="- Implement automated testing (no test/spec directory detected)\n"
    fi
    if [ ! -d "$PROJECT_PATH/.github/workflows" ] && ! find "$PROJECT_PATH" -maxdepth 1 -iname ".gitlab-ci.yml" -o -maxdepth 1 -iname ".travis.yml" 2>/dev/null | grep -q .; then
        recommendations+="- Set up CI/CD pipeline (no CI configuration detected)\n"
    fi
    [ -z "$recommendations" ] && recommendations="- No gaps detected against baseline checks (docs, tests, CI) — keep it up.\n"
...
main() {
    ...
    # Note: FRAMEWORKS_CACHE must be set before PROJECT_TYPE_CACHE
    # (detect_project_type reads ${FRAMEWORKS_CACHE:-$(detect_all_frameworks)})
    ALL_LANGUAGES_CACHE=$(detect_all_languages)
    FRAMEWORKS_CACHE=$(detect_all_frameworks)
    PROJECT_TYPE_CACHE=$(detect_project_type)

    # Count file types once and reuse across all generators
    mapfile -t FILE_COUNTS < <(count_file_types)
    FILE_COUNTS_SC="${FILE_COUNTS[0]}"
    FILE_COUNTS_DOC="${FILE_COUNTS[1]}"
    FILE_COUNTS_CFG="${FILE_COUNTS[2]}"
