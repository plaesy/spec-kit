...
main() {
    ...
    # Resolve TARGET_DIR to an absolute path once, up front. Several
    # functions below (AI detection, show_cleanup_plan, remove_plaesy_directories,
    # create_backup) each independently `cd "$TARGET_DIR"` from the process's
    # current directory; since cd persists across calls in the same shell, a
    # relative TARGET_DIR (e.g. "foo/bar") would compound on every subsequent
    # cd and fail. Resolving once here keeps every later `cd "$TARGET_DIR"` a
    # no-op cd to the same place.
    if [[ -d "$TARGET_DIR" ]]; then
