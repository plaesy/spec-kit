...
case "$1" in
    "init")
        ...
        if [ ! -f "$PLAESY_HOME/scripts/plaesy-init.sh" ]; then
        ...
        "$PLAESY_HOME/scripts/plaesy-init.sh" "${@:2}"
        ;;
    "clean")
    ...
    "diagnose"|"doctor")
        ...
        INIT_SCRIPT="$PLAESY_HOME/scripts/plaesy-init.sh"
...
create_init_script() {
    ...
    # Copy the standalone plaesy-init.sh script from the repository
    if [ -f "$INSTALL_DIR/scripts/bash/plaesy-init.sh" ]; then
        cp "$INSTALL_DIR/scripts/bash/plaesy-init.sh" "$INSTALL_DIR/scripts/plaesy-init.sh"
        chmod +x "$INSTALL_DIR/scripts/plaesy-init.sh"
        print_success "Initialization script copied"
    else
        print_error "plaesy-init.sh script not found in $INSTALL_DIR/scripts/bash/"
    fi
