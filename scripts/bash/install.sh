...
# Single source of truth for the raw installer URL (was duplicated 6x below,
# each a place a stale URL could drift out of sync on a repo rename/move).
readonly PLAESY_INSTALL_URL="https://raw.githubusercontent.com/plaesy/spec-kit/main/scripts/bash/install.sh"
...
# ============================================================================
# HELPER FUNCTIONS - Common operations with logging
# ============================================================================
...
# Remove directory with validation and logging
safe_rmdir() {
    local dir="$1"
    local name="${2:-directory}"
    ...
    if [[ -d "$dir" ]]; then
        if rm -rf "$dir" 2>/dev/null; then
            log_success "Removed $name: $dir"
            return 0
        else
            log_error "Failed to remove $name: $dir"
            return 1
        fi
    fi
    return 0
...
download_framework() {
    ...
    if [[ -d "$INSTALL_DIR" ]]; then
        ...
        safe_rmdir "$INSTALL_DIR" "existing installation" || log_error "Cannot continue without cleanup"
    fi
    ...
    if ! git clone --depth 1 --quiet "$REPO_URL" "$temp_dir"; then
        rm -rf "$temp_dir"
        log_error "Failed to download framework from $REPO_URL"
    fi

    # Verify clone succeeded
    if [[ ! -f "$temp_dir/README.md" ]]; then
        rm -rf "$temp_dir"
        log_error "Downloaded framework is missing core files"
    fi
    ...
    if ! mv "$temp_dir"/* "$INSTALL_DIR"/; then
        rm -rf "$temp_dir"
        log_error "Failed to move framework to installation directory"
    fi
    ...
    if [[ ! -d "$INSTALL_DIR/scripts" ]] || [[ ! -d "$INSTALL_DIR/prompts" ]]; then
        log_error "Framework installation incomplete - missing critical directories"
    fi
...
# This generated CLI is a standalone file (written via a quoted heredoc, so it
# cannot see install.sh's own PLAESY_INSTALL_URL) — define the URL once here
# instead of repeating the literal at every curl call below.
readonly PLAESY_INSTALL_URL="https://raw.githubusercontent.com/plaesy/spec-kit/main/scripts/bash/install.sh"

# Download the installer to a temp file, sanity-check it, then execute it —
# never pipe curl straight into bash. This gives you a chance to inspect the
# script (and lets us reject a truncated/empty download) before it runs.
_plaesy_safe_remote_install() {
    local url="$PLAESY_INSTALL_URL"
    local tmp
    tmp=$(mktemp) || { echo "❌ Failed to create temporary file"; return 1; }
    ...
    if [ ! -s "$tmp" ] || ! head -1 "$tmp" | grep -q '^#!.*sh'; then
        echo "❌ Downloaded installer failed validation (empty or not a shell script)"
        rm -f "$tmp"
        return 1
    fi
...
if [ ! -d "$PLAESY_HOME" ] && [ "$1" != "install" ]; then
    ...
    echo "curl -fsSL $PLAESY_INSTALL_URL -o install.sh && bash install.sh"
    exit 1
fi
...
case "$1" in
    "init")
        ...
        if [ ! -f "$PLAESY_HOME/scripts/bash/plaesy-init.sh" ]; then
            ...
            _plaesy_safe_remote_install --repair
            exit 0
        fi
    ...
    "repair"|"fix")
        ...
        if ! curl -fsSL "$PLAESY_INSTALL_URL" -o "$temp_script"; then
