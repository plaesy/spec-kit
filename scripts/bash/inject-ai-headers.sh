...
find_prompt_files() {
    ...
    # Build the find(1) argument list as an array (never as a string) so patterns
    # and excludes cannot be interpreted as shell syntax, even if they contain
    # spaces, quotes, or glob metacharacters.
    local name_args=()
    ...
    for e in "${user_excludes[@]}"; do
        excl_args+=(-not -path "*/$e")
    done

    find "$target_dir" -type f \( "${name_args[@]}" \) "${excl_args[@]}"
}
...
get_file_description() {
    ...
    if [[ -z "$description" ]]; then
        ...
        case "$target_file" in
            ...
            *.instructions.md) description="Instructions: $filename" ;;
