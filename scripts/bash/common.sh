...
get_feature_paths() {
    ...
    # Use printf %q so every value is shell-escaped before it is ever eval'd/sourced —
    # repo paths and branch names are not trusted input (a crafted branch name could
    # otherwise break out of the assignment and inject commands into the caller's eval).
    printf 'REPO_ROOT=%q\n' "$repo_root"
