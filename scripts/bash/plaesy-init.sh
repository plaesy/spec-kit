...
select_ai_assistant() {
    ...
    echo "13. Local AI (Ollama/LM Studio)"
    echo "14. No AI (Manual development)"
    echo ""
    ...
    while true; do
        read -p "Choose your AI assistant (1-14): " choice
        case $choice in
            ...
            13) AI_CHOICE="local"; break ;;
            14) AI_CHOICE="none"; break ;;
            *) echo "Please enter a number between 1-14" ;;
        esac
    done
}
...
copy_core_framework() {
    ...
    case "$ai_choice" in
        "copilot"|"cursor"|"windsurf"|"claude"|"trae-ai"|"chatgpt"|"gemini"|"qwen-code"|"kilo-code"|"codex-cli"|"codex-web"|"opencode-cli"|"local")
            # All AI types need these scripts
            mkdir -p .plaesy/scripts
            cp "$PLAESY_HOME/scripts/common.sh" .plaesy/scripts/ 2>/dev/null || true
            cp "$PLAESY_HOME/scripts/check-task-prerequisites.sh" .plaesy/scripts/ 2>/dev/null || true
            cp "$PLAESY_HOME/scripts/create-new-feature.sh" .plaesy/scripts/ 2>/dev/null || true
            cp "$PLAESY_HOME/scripts/create-new-idea.sh" .plaesy/scripts/ 2>/dev/null || true
            cp "$PLAESY_HOME/scripts/create-new-plan.sh" .plaesy/scripts/ 2>/dev/null || true
            cp "$PLAESY_HOME/scripts/detect-secrets-local.sh" .plaesy/scripts/ 2>/dev/null || true
            cp "$PLAESY_HOME/scripts/get-feature-paths.sh" .plaesy/scripts/ 2>/dev/null || true
            cp "$PLAESY_HOME/scripts/update-agent-context.sh" .plaesy/scripts/ 2>/dev/null || true           
            ;;
        "none")
...
setup_ai_specific_config() {
    ...
    case "$AI_CHOICE" in
        "copilot")
            # GitHub Copilot configuration - ONLY .github folder
            mkdir -p ".github/prompts"
            mkdir -p ".github/chatmodes"
            
            # Copy existing chatmodes for GitHub Copilot
            cp -r ".plaesy/chatmodes/"* ".github/chatmodes/" 2>/dev/null || true

            # Copy prompt files from source instead of embedding
            cp -r ".plaesy/prompts/"* ".github/prompts/" 2>/dev/null || true
            
            # Copy copilot instructions from source file instead of embedding
            if [ -f ".plaesy/instructions/copilot-instructions.md" ]; then
                cp ".plaesy/instructions/copilot-instructions.md" ".github/copilot-instructions.md"            
            fi                   
            ;;
        ...
        "cursor")
            ...
            cp -r ".plaesy/chatmodes" ".cursor/" 2>/dev/null || true
            
            # Use existing copilot instructions with Cursor-specific adaptations
            if [ -f ".plaesy/instructions/copilot-instructions.md" ]; then
                # Copy and adapt copilot instructions for Cursor
                cp ".plaesy/instructions/copilot-instructions.md" ".cursor/instructions.md"
                
                # Add Cursor-specific header
                sed -i '1i# Cursor AI Instructions for Plaesy Spec-Kit\n\n> **Note**: This uses the same constitutional framework as GitHub Copilot with Cursor-specific adaptations.\n' ".cursor/instructions.md"
            else
                echo "Warning: .plaesy/instructions/copilot-instructions.md not found"
            fi
            
            # Copy existing prompts for Cursor AI
            if [ -d ".plaesy/prompts" ]; then
                mkdir -p ".cursor/prompts"
                cp ".plaesy/prompts/"*.md ".cursor/prompts/" 2>/dev/null || true
            fi
            ;;

        "claude")
            # Claude AI configuration - use existing copilot instructions
            mkdir -p ".claude"
            
            # Copy existing chatmodes for Claude AI
            cp -r ".plaesy/chatmodes" ".claude/" 2>/dev/null || true
            
            # Use existing copilot instructions with Claude-specific adaptations
            if [ -f ".plaesy/instructions/copilot-instructions.md" ]; then
                # Copy and adapt copilot instructions for Claude
                cp ".plaesy/instructions/copilot-instructions.md" ".claude/instructions.md"
                
                # Add Claude-specific header
                sed -i '1i# Claude AI Instructions for Plaesy Spec-Kit\n\n> **Note**: This uses the same constitutional framework as GitHub Copilot with Claude-specific adaptations.\n' ".claude/instructions.md"
            else
                echo "Warning: .plaesy/instructions/copilot-instructions.md not found"
            fi
            
            # Copy existing prompts for Claude AI
            if [ -d ".plaesy/prompts" ]; then
                mkdir -p ".claude/prompts"
                cp ".plaesy/prompts/"*.md ".claude/prompts/" 2>/dev/null || true
            fi
            ;;
        ...
        "windsurf")
            ...
            cp -r ".plaesy/chatmodes" ".windsurf/" 2>/dev/null || true
            
            # Use existing copilot instructions with Windsurf-specific adaptations
            if [ -f ".plaesy/instructions/copilot-instructions.md" ]; then
                # Copy and adapt copilot instructions for Windsurf
                cp ".plaesy/instructions/copilot-instructions.md" ".windsurf/instructions.md"
...
TRAE_REGISTRY_EOF
        ...
        "qwen-code")
            ...
            # Copy existing chatmodes for Qwen Code
            cp -r ".plaesy/chatmodes/"* ".qwen/plaesy-method/" 2>/dev/null || true
...
QWEN_INSTRUCTIONS_EOF
        ...
        "kilo-code")
            ...
            # Copy existing chatmodes for easier access
            mkdir -p ".kilo/plaesy"
            cp -r ".plaesy/chatmodes/"* ".kilo/plaesy/" 2>/dev/null || true
...
OPENCODE_CONFIG_EOF
            ...
            mkdir -p ".opencode"
            cat > ".opencode/plaesy-instructions.yaml" << 'OPENCODE_INSTRUCTIONS_EOF'
...
OPENCODE_INSTRUCTIONS_EOF
        ...
        "chatgpt"|"gemini"|"local")
            ...
            cp -r ".plaesy/chatmodes" ".ai-config/" 2>/dev/null || true
            
            # Use existing copilot instructions with generic AI adaptations
            if [ -f ".plaesy/instructions/copilot-instructions.md" ]; then
                # Copy and adapt copilot instructions for generic AI
                cp ".plaesy/instructions/copilot-instructions.md" ".ai-config/instructions.md"
...
print_completion() {
    ...
    case "$AI_CHOICE" in
        "copilot")
            echo "• .github/prompts/ - GitHub Copilot specific commands"
            echo "• .github/chatmodes/ - GitHub Copilot specific chatmodes"
            echo "• .github/copilot-instructions.md - GitHub Copilot specific instructions"
            echo ""
            echo -e "${BLUE}GitHub Copilot users:${NC}"
            echo "• Use /idea, /specify, /plan, /tasks commands in chat"
            echo "• Framework rules in .plaesy/memory/constitution.md"
            echo "• Templates in .plaesy/templates/"
            ;;
        "cursor")
            echo "• .cursor/commands/ - Cursor AI specific commands"
            echo ""
            echo -e "${BLUE}Cursor AI users:${NC}"
            echo "• Load commands from .cursor/commands/"
            echo "• Framework rules in .plaesy/memory/constitution.md"
            echo "• Templates in .plaesy/templates/"
            ;;
        "claude")
            echo "• .claude/commands/ - Claude AI specific commands"
            echo ""
            echo -e "${BLUE}Claude AI users:${NC}"
            echo "• Load commands from .claude/commands/"
            echo "• Framework rules in .plaesy/memory/constitution.md"
            echo "• Templates in .plaesy/templates/"
            ;;
        "chatgpt"|"gemini"|"local")
