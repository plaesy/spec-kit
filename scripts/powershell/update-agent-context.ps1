...
function Initialize-AgentFile($targetFile, $agentName) {
    ...
    $template = Join-Path $repoRoot '.plaesy/templates/agent-file-template.md'
...
function Update-AgentFile($targetFile, $agentName) {
    ...
    $manualAdditions = $null
    if ($content -match '(?s)<!-- MANUAL ADDITIONS START -->.*?<!-- MANUAL ADDITIONS END -->') {
        $manualAdditions = $matches[0]
        $content = $content.Replace($manualAdditions, '')
    }
    ...
    if ($manualAdditions) { $content = $content.TrimEnd() + "`n`n" + $manualAdditions + "`n" }

    $content | Set-Content $targetFile -Encoding UTF8
    Write-Output "[SUCCESS] $agentName context file updated successfully"
}
