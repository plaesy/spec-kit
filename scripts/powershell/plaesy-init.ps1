...
function Set-AISpecificConfig {
    ...
    try {
        ...
        if ($platformConfig -and $platformConfig.mapping) {
            ...
            # Note: instructions are already copied to .plaesy/instructions/ by Copy-Instructions (universal, not platform-specific)
...
function Copy-Instructions {
    ...
    Log-Info "Copying instructions to .plaesy/instructions/..."
    $instructionsDir = Join-Path $TargetDir "instructions"
    $null = New-Item -ItemType Directory -Force -Path $instructionsDir

    $mappingFile = Join-Path $sourceDir "mapping.json"

    # Match the bash twin's copy_instructions(): prefer detect-stack.ps1's
    # combined always_load + stack-matched list (e.g. docx/xlsx/pptx
    # instructions when the project contains Office files); fall back to
    # always_load alone if detection is unavailable or finds nothing, unless
    # -AllInstructions was requested, in which case every instruction file
    # in the source directory is copied.
    $instructionsToCopy = $null

    if ($AllInstructions) {
        $instructionsToCopy = Get-ChildItem -Path $sourceDir -Filter "*.instructions.md" -File -ErrorAction SilentlyContinue |
            ForEach-Object { $_.Name }
    } else {
        $detectScript = Join-Path $ScriptDir "detect-stack.ps1"
        if (Test-Path $detectScript) {
            try {
                $instructionsToCopy = & $detectScript -TargetDir "." -PlaesyRoot (Join-Path $ScriptDir "..\..")
            }
            catch {
                Write-Warning "detect-stack.ps1 failed, falling back to always_load: $_"
                $instructionsToCopy = $null
            }
        }
    }

    if (-not $instructionsToCopy -or $instructionsToCopy.Count -eq 0) {
        if (Test-Path $mappingFile) {
            $config = Get-Content $mappingFile -Raw | ConvertFrom-Json -ErrorAction SilentlyContinue
            $instructionsToCopy = $config.mappings.always_load
        }
    }

    if ($instructionsToCopy) {
        foreach ($file in $instructionsToCopy) {
            $src = Join-Path $sourceDir $file
            if (Test-Path $src) {
                $name = $file -replace '\.instructions\.md$', '.md'
                $dst = Join-Path $instructionsDir $name
                ...
                Write-Warning "  ? $file (source not found, skipping)"
            }
        }
    }
...
function Copy-Templates {
    ...
    $sourceDir = Join-Path $ScriptDir "..\.." "templates"
    if (-not (Test-Path $sourceDir)) {
        Write-Warning "Templates source directory not found: $sourceDir"
        return $false
    }

    Log-Info "Copying templates to .plaesy/templates/..."
    $templatesDir = Join-Path $TargetDir "templates"
    $null = New-Item -ItemType Directory -Force -Path $templatesDir

    foreach ($filter in "*.md","*.json","*.yaml") {
        Get-ChildItem -Path $sourceDir -File -Filter $filter -ErrorAction SilentlyContinue | ForEach-Object {
            $targetPath = Join-Path $templatesDir $_.Name
            if (-not (Test-Path $targetPath)) {
                Copy-Item -Path $_.FullName -Destination $targetPath -Force
            }
        }
...
function Copy-Checklists {
    [CmdletBinding()]
    param([Parameter(Mandatory=$true)][string]$TargetDir)

    $sourceDir = Join-Path $ScriptDir "..\.." "checklists"
    if (-not (Test-Path $sourceDir)) {
        Write-Warning "Checklists source directory not found: $sourceDir"
        return $false
    }

    Log-Info "Copying checklists to .plaesy/checklists/..."
    $checklistsDir = Join-Path $TargetDir "checklists"
    $null = New-Item -ItemType Directory -Force -Path $checklistsDir

    Get-ChildItem -Path $sourceDir -Filter "*.md" -ErrorAction SilentlyContinue | ForEach-Object {
        $targetPath = Join-Path $checklistsDir $_.Name
        if (-not (Test-Path $targetPath)) {
            Copy-Item -Path $_.FullName -Destination $targetPath -Force
        }
    }

    Write-Success "Checklists copied"
}
...
function New-MemoryStructure {
    ...
    # ($AnalysisDir = ".plaesy/analysis") — flat, matching the
...
function Main {
    ...
    Copy-Templates -TargetDir ".plaesy"
    Copy-Checklists -TargetDir ".plaesy"
