...
# git writes informational messages ("Switched to a new branch...") to stderr,
# which PS 5.1 promotes to a terminating NativeCommandError when
# $ErrorActionPreference='Stop'. Temporarily relax EAP around native git calls.
$eap = $ErrorActionPreference
$ErrorActionPreference = 'Continue'
git checkout -b $branchName 2>$null
$gitExitCode = $LASTEXITCODE
$ErrorActionPreference = $eap
if ($gitExitCode -ne 0) { exit $gitExitCode }
