...
$InstallerRawUrl = "https://raw.githubusercontent.com/plaesy/spec-kit/main/scripts/powershell/install.ps1"
...
function Get-RemoteVersion {
    try {
        $response = Invoke-RestMethod -Uri "https://raw.githubusercontent.com/plaesy/spec-kit/main/VERSION" -UseBasicParsing -ErrorAction Stop
        $version = $response -split "`n" | Select-Object -First 1 | ForEach-Object { $_.Trim() }
...
function New-SafeDirectory {
    ...
    if (!(Test-Path $Path)) {
        try {
            ...
            Write-Success "Created $Description : $Path"
            return $true
        } catch {
            Write-Error-Custom "Failed to create $Description : $Path - $($_.Exception.Message)"
            return $false
        }
    }
    return $true
...
function Remove-SafeDirectory {
    ...
    if (Test-Path $Path) {
        try {
            ...
            Write-Error-Custom "Failed to remove $Description : $Path - $($_.Exception.Message)"
            return $false
        }
    }
    return $true
...
function Install-Framework {
    ...
    if (Test-Path $InstallPath) {
        if ($Force) {
            ...
            if (!(Remove-SafeDirectory -Path $InstallPath -Description "existing installation")) {
                Write-Error-Custom "Cannot continue without cleanup"
            }
        }
        else {
...
set "PLAESY_INSTALL_URL=https://raw.githubusercontent.com/plaesy/spec-kit/main/scripts/powershell/install.ps1"

if not exist "%PLAESY_HOME%" if not "%1"=="install" (
    echo Plaesy framework not found. Please run the installer first.
    echo iwr -useb %PLAESY_INSTALL_URL% -OutFile install.ps1 ^&^& powershell -ExecutionPolicy Bypass -File install.ps1
    exit /b 1
)
...
:safe_remote_install
REM Download the installer to a temp file, sanity-check it, then execute it —
REM never pipe iwr straight into iex. This gives a chance to inspect the
REM script (and rejects a truncated/empty download) before it runs.
setlocal
set "TMP_INSTALL=%TEMP%\plaesy-install-%RANDOM%.ps1"
powershell -NoProfile -Command "try { Invoke-WebRequest -Uri '%PLAESY_INSTALL_URL%' -UseBasicParsing -OutFile '%TMP_INSTALL%' -ErrorAction Stop } catch { exit 1 }"
if errorlevel 1 (
    echo Failed to download installer from %PLAESY_INSTALL_URL%
    endlocal
    exit /b 1
)
if not exist "%TMP_INSTALL%" (
    echo Downloaded installer not found
    endlocal
    exit /b 1
)
for %%A in ("%TMP_INSTALL%") do if %%~zA==0 (
    echo Downloaded installer failed validation ^(empty file^)
    del /f /q "%TMP_INSTALL%" >nul 2>&1
    endlocal
    exit /b 1
)
...
if "%1"=="init" (
    REM Check if we have enough arguments for auto-detection like bash
    if "%2"=="" (
        REM No directory specified - default to current directory with auto-detection
        powershell -ExecutionPolicy Bypass -File "%PLAESY_HOME%\scripts\powershell\plaesy-init.ps1" .
        goto :eof
    )
    if "%2"=="." (
        REM Current directory specified - use auto-detection like bash
        powershell -ExecutionPolicy Bypass -File "%PLAESY_HOME%\scripts\powershell\plaesy-init.ps1" .
        goto :eof
    )
    if "%3"=="-AI" (
        REM PowerShell format with -AI parameter
        powershell -ExecutionPolicy Bypass -File "%PLAESY_HOME%\scripts\powershell\plaesy-init.ps1" %2 %3 %4
        goto :eof
    )
    if "%3"=="-ai" (
        REM PowerShell format with -ai parameter
    ...
    if "%3"=="--ai" (
        REM bash format with --ai parameter
        powershell -ExecutionPolicy Bypass -File "%PLAESY_HOME%\scripts\powershell\plaesy-init.ps1" %2 %3 %4
        goto :eof
    )
    REM Default case - treat as directory with auto-detection
    powershell -ExecutionPolicy Bypass -File "%PLAESY_HOME%\scripts\powershell\plaesy-init.ps1" %*
    goto :eof
)

if "%1"=="clean" (
    powershell -ExecutionPolicy Bypass -File "%PLAESY_HOME%\scripts\powershell\plaesy-helper.ps1" -Command clean %*
    goto :eof
)

if "%1"=="analyze" (
    REM Check if plaesy-analyze.ps1 exists, if not, offer to fix it
    if not exist "%PLAESY_HOME%\scripts\powershell\plaesy-analyze.ps1" (
        ...
        call :safe_remote_install
        goto :eof
    )

    REM Run project analysis (drop the leading "analyze" subcommand,
    REM otherwise plaesy-analyze.ps1 binds it to -ProjectPath)
    shift
    powershell -ExecutionPolicy Bypass -File "%PLAESY_HOME%\scripts\powershell\plaesy-analyze.ps1" %*
    goto :eof
)
...
if exist "%PLAESY_HOME%" (
    ...
    echo   plaesy status        - Check installation status and show system information
) else (
...
echo Plaesy Spec-Kit Status Check
echo.
...
echo.
echo Examples:
echo   plaesy init .                    # Initialize in current directory (auto-detects platform)
echo   plaesy init my-project           # Initialize in new directory (auto-detects platform)
echo   plaesy init . -AI claude_code    # Initialize with Claude Code
echo   plaesy init . -AI cursor_ai      # Initialize with Cursor AI
echo   plaesy init . -AI github_copilot # Initialize with GitHub Copilot
...
function Copy-PlatformDetector {
    Write-Step "Copying platform detector..."

    # Copy platform detector to framework directory
    $detectorSource = $PSScriptRoot + "\platform-detector.ps1"
    $detectorDest = $InstallPath + "\scripts\powershell\platform-detector.ps1"
    $helperSource = $PSScriptRoot + "\plaesy-helper.ps1"
    $helperDest = $InstallPath + "\scripts\powershell\plaesy-helper.ps1"

    if (Test-Path $detectorSource) {
        Copy-Item -Path $detectorSource -Destination $detectorDest -Force
        Write-Success "Platform detector copied"
    } else {
        Write-Warning "Platform detector not found, creating basic detector"
        # Create basic platform detector as fallback
        $basicDetector = @'
# Basic platform detector fallback
function Get-DetectedAiPlatform {
    # Claude Code
    if ((Test-Path ".claude\settings.local.json") -or (Test-Path "CLAUDE.md")) {
        return "claude"
    }

    # Cursor AI
    if ((Test-Path ".cursorrules") -or (Test-Path ".cursor\rules")) {
        return "cursor"
    }

    # Windsurf AI
    if (Test-Path ".windsurf\workflow.yaml") {
        return "windsurf"
    }
...
function Get-PlatformCommand {
    param([string]$Platform, [string]$Command)

    switch ($Platform) {
        "claude" { return "/$Command" }
        "cursor" { return "--$Command" }
        default { return $Command }
    }
}
'@
        $basicDetector | Out-File -FilePath $detectorDest -Encoding UTF8
        Write-Success "Basic platform detector created"
    }

    # Copy helper script
    if (Test-Path $helperSource) {
        Copy-Item -Path $helperSource -Destination $helperDest -Force
        Write-Success "Plaesy helper copied"
    } else {
        Write-Warning "Plaesy helper not found in $($PSScriptRoot)"
    }
}
...
function New-InitScript {
    ...
    $bashScriptsDir = Join-Path -Path $InstallPath -ChildPath "scripts\bash"
    $powershellScriptsDir = Join-Path -Path $InstallPath -ChildPath "scripts\powershell"

    New-Item -ItemType Directory -Path $bashScriptsDir -Force | Out-Null
    New-Item -ItemType Directory -Path $powershellScriptsDir -Force | Out-Null
    Write-Success "Platform-specific script directories ready (bash, powershell)"

    # Verify all bash scripts
    if (Test-Path $bashScriptsDir) {
        $bashScripts = @(Get-ChildItem -Path $bashScriptsDir -Filter "*.sh" -ErrorAction SilentlyContinue)
        if ($bashScripts.Count -gt 0) {
            Write-Success "Bash scripts ready ($($bashScripts.Count) files)"
        } else {
            Write-Warning "No bash scripts found (optional)"
        }
    }

    # Verify all PowerShell scripts
    if (Test-Path $powershellScriptsDir) {
        $psScripts = @(Get-ChildItem -Path $powershellScriptsDir -Filter "*.ps1" -ErrorAction SilentlyContinue)
        if ($psScripts.Count -gt 0) {
            Write-Success "PowerShell scripts ready ($($psScripts.Count) files)"
        } else {
            Write-Error-Custom "No PowerShell scripts found"
        }
    }
}
...
function Set-PathEnvironment {
    ...
    if ($currentPath -notlike "*$BinDir*") {
        ...
        try {
            ...
            if ($verifyPath -like "*$BinDir*") {
                # Update current session PATH immediately
                $env:PATH = "$env:PATH;$BinDir"
                Write-Success "PATH environment variable updated"
            } else {
                Write-Error-Custom "Failed to update PATH environment variable"
            }
        } catch {
            Write-Error-Custom "Failed to set PATH: $($_.Exception.Message)"
        }
    }
    else {
    ...
    try {
        ...
        Write-Warning "Could not refresh environment automatically: $($_.Exception.Message)"
    }
}
...
function Start-Uninstall {
    ...
    Write-Host "iwr -useb $InstallerRawUrl -OutFile install.ps1; powershell -ExecutionPolicy Bypass -File install.ps1"
    Write-Host ""
}
