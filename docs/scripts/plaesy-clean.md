...
"claude_code": {
  "mapping": {
    ...
    "chatmodes": ".claude/roles"           → Listed, but never actually removed (see note)
  }
}
```

> **Note (verified against `scripts/bash/plaesy-clean.sh` 2026-09-16)**: `chatmodes`
> is included in the script's `mapping_types` array and it does check
> `.claude/roles` for removal, but `plaesy-init.sh` never creates that path —
> chatmodes always install to `.plaesy/roles/` regardless of platform (see the
> matching discrepancy note in `docs/scripts/plaesy-init.md`). Since the removal
> code only acts `if [[ -d "$target_path" ]]`, this check is always false for
> `chatmodes` and the "Removed" line never actually prints for it — harmless
> no-op, but the example output further below overstates what happens.
...
Platform-Specific Files (claude_code):
  ...
  (.claude/roles/ is not listed — chatmodes actually installed to .plaesy/roles/,
  which the ".plaesy/" line above already covers; see note above)
...
[INFO] Removing platform-specific files for: claude_code
  ...
  (no line for chatmodes: .claude/roles was never created — chatmodes live under
  .plaesy/roles/, already removed with .plaesy above — see note earlier)
