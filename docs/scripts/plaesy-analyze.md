...
**Note**: `context.md` and `memory.md` are NOT generated by `plaesy analyze` — they are manually curated project files that live at `.plaesy/context.md` and `.plaesy/memory.md`. Topic-specific memory files (`.plaesy/memory/[topic].md`) are also user-created. The `plaesy analyze` command only generates output files listed below.
...
### 3. `overview.md` - Analysis Snapshot
**Location**: `.plaesy/analysis/overview.md`

A concise, AI-readable project overview that is fully replaced on every `plaesy analyze` run. Manual session notes belong in `.plaesy/context.md` and `.plaesy/memory.md`, which analyze does not modify.

### 4. Dependency Graph (from `plaesy-graph`, built automatically)
...
4. **Read project.graph.json / reports.md**: Understand cross-file dependencies, communities, and high-degree files
5. **Proceed with development**: Use analysis for context-aware work

**Note:** `context.md` and `memory.md` at `.plaesy/context.md` and `.plaesy/memory.md` are manually curated project files — they are NOT generated or modified by `plaesy analyze`.
...
4. Proceed with context-aware development
```

**Manual Context Files (Not Generated by Analyze):**
- `.plaesy/context.md` — Session context and current work state (manually maintained)
- `.plaesy/memory.md` — Project knowledge index (manually maintained)
- `.plaesy/memory/[topic].md` — Topic-specific memory files (user-created)
