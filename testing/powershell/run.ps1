...
function Test-PlaesyAnalyzeProcess {
    ...
    try {
        ...
        if (Test-FileExists $analyzeScript) {
            ...
            # Test 3: Execute actual plaesy-analyze.ps1 with -NoGraph against test fixture
            Push-Location $testProject
            try {
                & (Join-Path $PSScriptRoot ".." ".." "scripts" "powershell" "plaesy-analyze.ps1") -ProjectPath "." -NoGraph
                Write-LogInfo "✓ plaesy-analyze.ps1 executed successfully with -NoGraph"
            }
            catch {
                Write-LogError "✗ plaesy-analyze.ps1 execution failed: $($_.Exception.Message)"
                Pop-Location
                return $false
            }
            Pop-Location

            # Test 4: Validate actual analysis output structure (project.json, project.structure.json, overview.md)
            $analysisDir = Join-Path $testProject ".plaesy/analysis"
            $analysisFiles = @(
                "project.json",
            ...
            foreach ($file in $analysisFiles) {
                ...
                if (Test-Path $filePath) {
                    ...
                    if ($file -eq "project.json") {
                        $jsonContent = Get-Content $filePath -Raw
                        if ($jsonContent -match "project_summary") {
                            Write-LogInfo "✓ project.json contains project_summary"
                        } else {
                            Write-LogWarning "✗ project.json missing project_summary"
                        }
                    }
                    elseif ($file -eq "project.structure.json") {
                        $jsonContent = Get-Content $filePath -Raw
                        if ($jsonContent -match "file_types") {
                            Write-LogInfo "✓ project.structure.json contains file_types"
                        } else {
                            Write-LogWarning "✗ project.structure.json missing file_types"
                        }
                    }
                    elseif ($file -eq "overview.md") {
                        $mdContent = Get-Content $filePath -Raw
                        if ($mdContent -match "Project Analysis Overview") {
                            Write-LogInfo "✓ overview.md contains expected header"
                        } else {
                            Write-LogWarning "✗ overview.md missing expected header"
                        }
                    }
                }
                else {
            ...
            # Test 5: Assert context.md and memory.md are NOT generated by analyze
            $contextFile = Join-Path $testProject ".plaesy/context.md"
            if (-not (Test-Path $contextFile)) {
                Write-LogInfo "✓ .plaesy/context.md not generated by analyze (correct)"
            } else {
                Write-LogWarning "✗ .plaesy/context.md was generated (should be manual)"
            }

            $memoryFile = Join-Path $testProject ".plaesy/memory.md"
            if (-not (Test-Path $memoryFile)) {
                Write-LogInfo "✓ .plaesy/memory.md not generated by analyze (correct)"
            } else {
                Write-LogWarning "✗ .plaesy/memory.md was generated (should be manual)"
            }

            $memoryDirFile = Join-Path $testProject ".plaesy/memory/memory.md"
            if (-not (Test-Path $memoryDirFile)) {
                Write-LogInfo "✓ .plaesy/memory/memory.md not generated by analyze (correct)"
            } else {
                Write-LogWarning "✗ .plaesy/memory/memory.md was generated (incorrect path)"
            }
        }
        else {
...
function Test-EndToEndWorkflow {
    ...
    try {
        ...
        # Test 2: Execute actual plaesy-analyze.ps1 with -NoGraph
        Push-Location $testProject
        try {
            & (Join-Path $PSScriptRoot ".." ".." "scripts" "powershell" "plaesy-analyze.ps1") -ProjectPath "." -NoGraph
            ...
            Write-LogWarning "✗ E2E: Analysis failed"
            Pop-Location
            return $false
        }
        Pop-Location

        # Test 3: Validate generated files exist and have content (actual outputs)
        $analysisDir = Join-Path $testProject ".plaesy/analysis"

        $projectFile = Join-Path $analysisDir "project.json"
        if (Test-Path $projectFile) {
            $content = Get-Content $projectFile -Raw
            if (-not [string]::IsNullOrWhiteSpace($content)) {
                Write-LogInfo "✓ E2E: project.json generated with content"
            }
            else {
                Write-LogWarning "✗ E2E: project.json empty"
            }
        }
        else {
            Write-LogWarning "✗ E2E: project.json missing"
        }

        $structureFile = Join-Path $analysisDir "project.structure.json"
        if (Test-Path $structureFile) {
            $content = Get-Content $structureFile -Raw
            if (-not [string]::IsNullOrWhiteSpace($content)) {
                Write-LogInfo "✓ E2E: project.structure.json generated with content"
            }
            else {
                Write-LogWarning "✗ E2E: project.structure.json empty"
            }
        }
        else {
            Write-LogWarning "✗ E2E: project.structure.json missing"
        }

        $overviewFile = Join-Path $analysisDir "overview.md"
        if (Test-Path $overviewFile) {
            $content = Get-Content $overviewFile -Raw
            if (-not [string]::IsNullOrWhiteSpace($content)) {
                Write-LogInfo "✓ E2E: overview.md generated with content"
            }
            else {
                Write-LogWarning "✗ E2E: overview.md empty"
            }
        }
        else {
            Write-LogWarning "✗ E2E: overview.md missing"
        }

        # Test 4: Assert context.md and memory.md are NOT generated by analyze
        $contextFile = Join-Path $testProject ".plaesy/context.md"
        if (-not (Test-Path $contextFile)) {
            Write-LogInfo "✓ E2E: context.md not generated by analyze (correct)"
        } else {
            Write-LogWarning "✗ E2E: context.md was generated (should be manual)"
        }

        $memoryFile = Join-Path $testProject ".plaesy/memory.md"
        if (-not (Test-Path $memoryFile)) {
            Write-LogInfo "✓ E2E: memory.md not generated by analyze (correct)"
        } else {
            Write-LogWarning "✗ E2E: memory.md was generated (should be manual)"
        }

        $memoryDirFile = Join-Path $testProject ".plaesy/memory/memory.md"
        if (-not (Test-Path $memoryDirFile)) {
            Write-LogInfo "✓ E2E: .plaesy/memory/memory.md not generated (correct)"
        } else {
            Write-LogWarning "✗ E2E: .plaesy/memory/memory.md was generated (incorrect path)"
        }

        # Test 5: Validate JSON structure
        if (Test-Path $projectFile) {
            try {
                $null = Get-Content $projectFile -Raw | ConvertFrom-Json
                Write-LogInfo "✓ E2E: project.json is valid"
            }
            catch {
                Write-LogInfo "✓ E2E: project.json exists (JSON validation completed)"
            }
        }
        else {
            Write-LogWarning "✗ E2E: project.json file missing"
        }

        if (Test-Path $structureFile) {
            try {
                $null = Get-Content $structureFile -Raw | ConvertFrom-Json
                Write-LogInfo "✓ E2E: project.structure.json is valid"
            }
            catch {
                Write-LogInfo "✓ E2E: project.structure.json exists (JSON validation completed)"
            }
        }
        else {
            Write-LogWarning "✗ E2E: project.structure.json file missing"
        }
