...
# Test plaesy-analyze functionality
            if check_file_exists "$FRAMEWORK_ROOT/scripts/bash/plaesy-analyze.sh"; then
                if timeout 15 bash "$FRAMEWORK_ROOT/scripts/bash/plaesy-analyze.sh" "$test_project" --no-graph >/dev/null 2>&1; then
            log_info "✓ plaesy-analyze processes project successfully"

            # Check if analysis files were created (actual outputs: project.json, project.structure.json, overview.md)
            local analysis_files=(
                "$test_project/.plaesy/analysis/project.json"
                "$test_project/.plaesy/analysis/project.structure.json"
                "$test_project/.plaesy/analysis/overview.md"
            )
            ...
            for file in "${analysis_files[@]}"; do
                if [[ -f "$file" ]]; then
                    ...
                    elif [[ "$file" == *"overview.md" ]]; then
                        if grep -q "Project Analysis Overview" "$file"; then
                            log_info "✓ overview.md contains expected header"
                        else
                            log_warning "✗ overview.md missing expected header"
                        fi
                    fi
                else
            ...
            # Assert context.md and memory.md are NOT generated by analyze
            if [[ ! -f "$test_project/.plaesy/context.md" ]]; then
                log_info "✓ .plaesy/context.md not generated by analyze (correct)"
            else
                log_warning "✗ .plaesy/context.md was generated (should be manual)"
            fi

            if [[ ! -f "$test_project/.plaesy/memory.md" ]]; then
                log_info "✓ .plaesy/memory.md not generated by analyze (correct)"
            else
                log_warning "✗ .plaesy/memory.md was generated (should be manual)"
            fi

            if [[ ! -f "$test_project/.plaesy/memory/memory.md" ]]; then
                log_info "✓ .plaesy/memory/memory.md not generated by analyze (correct)"
            else
                log_warning "✗ .plaesy/memory/memory.md was generated (incorrect path)"
            fi

            # test-runner.sh is NOT generated by analyze (generate_project_scripts not called)
            if [[ ! -f "$test_project/scripts/test-runner.sh" ]]; then
                log_info "✓ test-runner.sh not generated by analyze (correct)"
            else
                log_warning "✗ test-runner.sh was generated (should not be)"
            fi
        else
...
test_end_to_end_workflow() {
    ...
    local project_file="$test_project/.plaesy/analysis/project.json"
    if [[ -f "$project_file" ]] && [[ -s "$project_file" ]]; then
        log_info "✓ E2E: project.json generated with content"
    else
        log_warning "✗ E2E: project.json missing or empty"
    fi

    local structure_file="$test_project/.plaesy/analysis/project.structure.json"
    if [[ -f "$structure_file" ]] && [[ -s "$structure_file" ]]; then
        log_info "✓ E2E: project.structure.json generated with content"
    else
        log_warning "✗ E2E: project.structure.json missing or empty"
    fi

    local overview_file="$test_project/.plaesy/analysis/overview.md"
    if [[ -f "$overview_file" ]] && [[ -s "$overview_file" ]]; then
        log_info "✓ E2E: overview.md generated with content"
    else
        log_warning "✗ E2E: overview.md missing or empty"
    fi

    # Test 4: Assert context.md and memory.md are NOT generated by analyze
    local context_file="$test_project/.plaesy/context.md"
    if [[ ! -f "$context_file" ]]; then
        log_info "✓ E2E: context.md not generated by analyze (correct)"
    else
        log_warning "✗ E2E: context.md was generated (should be manual)"
    fi

    local memory_file="$test_project/.plaesy/memory.md"
    if [[ ! -f "$memory_file" ]]; then
        log_info "✓ E2E: memory.md not generated by analyze (correct)"
    else
        log_warning "✗ E2E: memory.md was generated (should be manual)"
    fi

    local memory_dir_file="$test_project/.plaesy/memory/memory.md"
    if [[ ! -f "$memory_dir_file" ]]; then
        log_info "✓ E2E: .plaesy/memory/memory.md not generated (correct)"
    else
        log_warning "✗ E2E: .plaesy/memory/memory.md was generated (incorrect path)"
    fi
